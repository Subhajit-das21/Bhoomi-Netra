    ...
    <div className="h-full bg-[#0B1120] text-slate-200 p-6 flex flex-col font-sans">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">Hardware status and live telemetry</p>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-3 gap-6 min-h-0">
        ...
        <div className="xl:col-span-2 bg-[#111827] border border-slate-700/50 rounded-xl shadow-lg flex flex-col overflow-hidden">
          <div className="p-4 bg-[#151D2C] border-b border-slate-700/50 flex justify-between items-center">
            <h2 className="text-sm font-bold text-slate-200 uppercase tracking-wider">Active Nodes</h2>
          </div>
          
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-[#1A2332] text-slate-400">
                <tr>
              ...
              <tbody className="divide-y divide-slate-700/50">
                {mockNodes.map((node) => (
                  <tr 
                    ...
                    className={`cursor-pointer transition-colors hover:bg-[#1E293B] ${selectedNode?.id === node.id ? 'bg-[#1E293B]' : ''}`}
                  >
                    <td className="px-6 py-4 font-bold text-white">{node.id}</td>
                    <td className="px-6 py-4 text-slate-300">{node.location}</td>
                    ...
                    <td className="px-6 py-4 text-slate-300">
                      <div className="flex items-center gap-2">
                        {node.network === 'WiFi' ? <Wifi size={16} className="text-blue-400"/> : <Radio size={16} className="text-purple-400"/>}
                        {node.network}
                      </div>
                    </td>
                  </tr>
        ...
        {selectedNode && (
          <div className="bg-[#111827] border border-slate-700/50 rounded-xl p-5 shadow-lg flex flex-col overflow-y-auto">
            
            {/* Header */}
            <div className="border-b border-slate-700/50 pb-4 mb-4">
              <div className="flex justify-between items-start mb-2">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <Activity size={20} className="text-blue-400"/> NODE {selectedNode.id}
                </h2>
              ...
              <p className="text-sm text-slate-400">{selectedNode.location} • {selectedNode.type} Sensor</p>
            </div>

            {/* Vital Stats */}
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="bg-[#1A2332] p-3 rounded-lg border border-slate-700/50">
                <div className="text-slate-500 text-xs mb-1 flex items-center gap-1"><Battery size={12}/> Battery</div>
                <div className="text-lg font-bold text-white">{selectedNode.battery}%</div>
              </div>
              <div className="bg-[#1A2332] p-3 rounded-lg border border-slate-700/50">
                <div className="text-slate-500 text-xs mb-1 flex items-center gap-1"><Radio size={12}/> Network</div>
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between items-center bg-[#0B1120] p-3 rounded border border-slate-800">
                <span className="text-sm text-slate-400 flex items-center gap-2"><Droplets size={16}/> Water Level</span>
                <span className="font-bold text-white">82 cm ⬆</span>
              </div>
              <div className="flex justify-between items-center bg-[#0B1120] p-3 rounded border border-slate-800">
                <span className="text-sm text-slate-400 flex items-center gap-2"><CloudRain size={16}/> Rainfall</span>
                <span className="font-bold text-white">47 mm/hr</span>
              </div>
              <div className="flex justify-between items-center bg-[#0B1120] p-3 rounded border border-slate-800">
                <span className="text-sm text-slate-400 flex items-center gap-2"><Thermometer size={16}/> Temperature</span>
                <span className="font-bold text-white">31°C</span>
              </div>
              <div className="flex justify-between items-center bg-[#0B1120] p-3 rounded border border-slate-800">
                <span className="text-sm text-slate-400 flex items-center gap-2"><CloudFog size={16}/> Humidity</span>
                <span className="font-bold text-white">89%</span>
              </div>
            </div>

            <div className="mt-auto text-xs text-center text-slate-500 font-medium">
